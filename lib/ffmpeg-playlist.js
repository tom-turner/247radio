...
class Playlist {
  ...
  writeFirstTrack(name, data, next) {
    fs.writeFileSync(path.join(this.playlistsPath, `${name}.mp3`), data)
  ...
  removeTrack(name) {
    ...
    fs.unlinkSync(path.join(this.playlistsPath, `${name}.mp3`))
  }

  moveTrack(name, position) {
    const data = fs.readFileSync(path.join(this.playlistsPath, `${name}.mp3`))
    this.removeTrack(name)
    this.addTrack(name, data, position)
  }
}
